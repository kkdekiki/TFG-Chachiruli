...
def getObjectsSinLactosa():
    #Pagina de todas las categorias de alimentos sin lactosa, cojo los codigos de la categoria y monto urls con ellos. Esa url es pasada a otra funcion q extrae los objetos.
    res = []
    ...
    for element in listaCodes:
        ...
        HtmlCode = url1.read()
        SoupObject = BeautifulSoup(HtmlCode,"html.parser") #Se pone la segunda opcion para que no de errores
        resaux=(element[0],getObjetosDentroCat(SoupObject))
        res.append(resaux)
        print(resaux)
